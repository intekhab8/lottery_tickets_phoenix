...
def setOptimizerLRScheduler(optimizer, patience):
...
#       opt = optim.Adam(odenet.parameters(), lr=settings['init_lr'], weight_decay=settings['weight_decay'])
    ...
    torch.save(opt.state_dict(),"{}/optimizer.pt".format(init_model_save_dir))
    ...
    for epoch in range(1, tot_epochs + 1):
        ...
        if (settings['viz'] and epoch in viz_epochs) or (settings['viz'] and epoch in rep_epochs) or (consec_epochs_failed == epochs_to_fail_to_start_finetuning):
            print("Saving plot")
            with torch.no_grad():
                #print("nope..")
                visualizer.visualize()
        ...
        if  (consec_epochs_failed == epochs_to_fail_to_start_finetuning):
            ...
            print("So now we stop pruning, and simply finetune the best-val model so far")
            print("Load in the best val mask and initial weights")
            best_val_dict = torch.load("{}/best_val_model_dict.pt".format(output_root_dir), map_location=device)
            current_dict = odenet.state_dict()
            current_dict.update(best_val_dict)
