...
if __name__ == "__main__":
    ...
    #Plotting setup
    fig_breast_cancer = plt.figure(figsize=(4,15), tight_layout=True)
    gs = fig_breast_cancer.add_gridspec(10, 6, hspace = 0.03, wspace = 0.02) #
    border_width = 1.5
    tick_lab_size = 12
    ax_lab_size = 13
    ...
    ax = fig_breast_cancer.add_subplot(gs[0:10, 0:5])
...
#    fig_breast_cancer.savefig('{}/manuscript_fig_breast_cancer_permtest_{}.png'.format(output_root_dir, analysis_type), bbox_inches='tight')
    ...
    for tick, pad_value in zip(ax.xaxis.get_major_ticks(), [0,-8,0,-8,0]):
    ...
    cbar =  fig_breast_cancer.colorbar(c, ax= ax, use_gridspec = False,  
                            shrink=0.8, orientation = "horizontal", pad = 0.06)
    
    cbar.set_ticks([0 , 10, 20])
    cbar.set_ticklabels([r"$z$-score =0", '10', '20'], ha='right')
    cbar.ax.tick_params(labelsize = tick_lab_size+3) 
    cbar.outline.set_linewidth(2)
    
    
    fig_breast_cancer.savefig('{}/manuscript_fig_breast_cancer_permtest_{}_icml_straight.png'.format(output_root_dir, analysis_type),  bbox_inches='tight') #
